...
export default function AddTimeBlockModal({ isOpen, onClose, onSave, selectedDateTime }: AddTimeBlockModalProps) {
  ...
  useEffect(() => {
    if (selectedDateTime) {
      // Use UTC hours here for consistency
      const startHour = String(selectedDateTime.getUTCHours()).padStart(2, '0')
      const endHour = String(selectedDateTime.getUTCHours() + 1).padStart(2, '0')
  ...
  return (
    <div style={styles.overlay} onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="add-event-title">
      <div style={styles.modal} onClick={e => e.stopPropagation()}>
        <h2 id="add-event-title" style={styles.header}>Add Event</h2>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.timeGroup}>
            <div>
              ...
              <input id="startTime" type="time" value={startTime} onChange={e => setStartTime(e.target.value)} style={styles.input} required />
            </div>
            <div>
              <label htmlFor="endTime" style={styles.label}>End Time</label>
              <input id="endTime" type="time" value={endTime} onChange={e => setEndTime(e.target.value)} style={styles.input} required />
            </div>
          </div>
