...
export default function PricingCard({
  ...
  const handleYearlyPayment = async () => {
    ...
    if (!amount) {
      alert('Yearly pricing not available for this plan.');
      return;
    }

    // Create yearly payment order
    const orderResponse = await fetch('/api/razorpay/create-yearly-payment', {
      method: 'POST',
    ...
    const options = {
      ...
      name: 'NeuroNest',
      description: `${plan.name} Plan - Yearly Subscription`,
      handler: async function (response: any) {
        try {
          // Verify yearly payment
          const verifyResponse = await fetch('/api/razorpay/verify-yearly-payment', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
          ...
          if (verifyData.success) {
            window.location.href = '/dashboard?subscription=yearly-success';
          } else {
  ...
  const renderButton = () => {
    const baseClasses = `w-full py-3 px-6 rounded-lg font-semibold transition-all duration-200 ${loading === plan.name ? 'opacity-50 cursor-not-allowed' : ''
      }`;
    ...
    return (
      <button
        ...
        disabled={loading === plan.name}
        className={`${baseClasses} ${plan.popular
  ...
  return (
    <div
      className={`relative bg-slate-800 rounded-2xl p-8 ${plan.popular ? 'ring-2 ring-purple-500 scale-105' : ''
        }`}
    >
      {plan.popular && (
